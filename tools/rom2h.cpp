//Author: ackerman
//Convert NES .rom to .h Arduino compile NES ESP32 emulate
//input
// roms
//  .rom
//output
...
void WriteFileROM(unsigned char num,char *cadPath,char *cadFile, char *cadFileSource)
{
 ...
 sprintf(cadDest,"output/dataFlash/rom/gbrom%s.h",cadFile);
 auxWrite = fopen(cadDest,"w+");
 if (auxWrite!= NULL)
 {
  fprintf(auxWrite,"#ifndef _GB_ROM_%s_H\n",cadFile);
  fprintf(auxWrite," #define _GB_ROM_%s_H\n",cadFile);     
...
{//Los 48k
 ...
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) != 0)
   fprintf(gb_fileWrite," #include \"rom/gbrom%s.h\"\n",gb_titles[i]);
 }
 fprintf(gb_fileWrite,"\n");
 fprintf(gb_fileWrite," #define max_list_rom %d\n\n\n",gb_contRom);
 fprintf(gb_fileWrite," //rom\n //Titulos\n");
